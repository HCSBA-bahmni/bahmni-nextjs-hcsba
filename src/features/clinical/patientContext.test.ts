...
describe("clinical patient context", () => {
  it("maps the same patient profile contract used by registration", () => {
    const context = toClinicalPatientContext({ patient: { identifiers: [
      { identifier: "12345678-5", identifierType: { name: "RUN" } },
      { identifier: "HCSBA100001", preferred: true, identifierType: { name: "Patient Identifier" } },
      { identifier: "P-123", identifierType: { name: "Passport", display: "Pasaporte" } },
    ], person: { gender: "F", birthdateEstimated: true, birthtime: "1990-01-01T08:15:00.000-0300", names: [{ givenName: "Ana", familyName: "Pérez", familyName2: "Soto" }], addresses: [{ address1: "Calle 1", cityVillage: "Santiago" }], attributes: [{ attributeType: { name: "email", display: "Correo" }, value: "ana@example.org" }] } }, relationships: [{ uuid: "r1", personA: { uuid: "p1" }, personB: { uuid: "p2", display: "Juan Pérez" }, relationshipType: { aIsToB: "Hermana", bIsToA: "Hermano" } }] }, "p1");
    expect(context).toMatchObject({ uuid: "p1", name: "Ana Pérez Soto", identifier: "HCSBA100001", runIdentifier: "12345678-5", gender: "F", address: "Calle 1, Santiago" });
    ...
    expect(context.additionalIdentifiers).toEqual([{ name: "RUN", label: "RUN", value: "12345678-5" }, { name: "Passport", label: "Pasaporte", value: "P-123" }]);
