...
import { layoutCellKey, locationChildren, validateBedPosition, validateLayout, type AdminBed, type AdminBedType, type AdminLocation } from "./beds";
      ...
      <TabView>
        <TabPanel header="Ubicaciones de admisión" leftIcon="pi pi-sitemap mr-2">
          <div className="admin-location-layout">
            <aside aria-label="Ubicaciones de admisión"><header><strong>Ubicaciones de admisión</strong><Button icon="pi pi-plus" rounded text aria-label="Agregar ubicación de admisión" onClick={() => openLocation()} /></header><nav><button type="button" className={!selected ? "selected" : ""} onClick={() => setSelectedUuid(undefined)}>Todas las ubicaciones</button>{roots.map((root) => { const expanded = expandedLocations.has(root.uuid); return <div key={root.uuid}><button type="button" className={selectedUuid === root.uuid ? "selected" : ""} aria-expanded={expanded} aria-controls={`salas-${root.uuid}`} onClick={() => { setSelectedUuid(root.uuid); setExpandedLocations((current) => { const next = new Set(current); if (next.has(root.uuid)) next.delete(root.uuid); else next.add(root.uuid); return next; }); }}><i className={`pi ${expanded ? "pi-chevron-down" : "pi-chevron-right"}`} aria-hidden="true" /><i className="pi pi-building" aria-hidden="true" /> {root.name}</button>{expanded && <div id={`salas-${root.uuid}`}>{locationChildren(locations.data ?? [], root.uuid).map((ward) => <button type="button" className={`ward ${selectedUuid === ward.uuid ? "selected" : ""}`} key={ward.uuid} onClick={() => setSelectedUuid(ward.uuid)}><i className="pi pi-angle-right" aria-hidden="true" /> {ward.name}</button>)}</div>}</div>; })}</nav></aside>
            <div className="admin-location-main">
              {!selected && <><div className="admin-section-title"><div><h3>Ubicaciones de admisión</h3><p>Seleccione una ubicación para revisar sus salas.</p></div><Button label="Agregar ubicación" icon="pi pi-plus" onClick={() => openLocation()} /></div><div className="admin-location-cards">{roots.map((item) => <button key={item.uuid} type="button" onClick={() => setSelectedUuid(item.uuid)}><i className="pi pi-building" /><strong>{item.name}</strong>{item.description && <span>{item.description}</span>}</button>)}</div></>}
              {selected && !isWard && <><div className="admin-breadcrumb"><button onClick={() => setSelectedUuid(undefined)}>Ubicaciones de admisión</button><i className="pi pi-angle-right" /><span>{selected.name}</span></div><div className="admin-section-title"><div><h3>{selected.name}</h3><p>{selected.description || "Ubicación de admisión"}</p></div><div><Button outlined icon="pi pi-pencil" label="Editar" onClick={() => openLocation(selected)} /><Button outlined severity="danger" icon="pi pi-trash" label="Eliminar" onClick={() => window.confirm(`¿Está seguro de que desea eliminar la ubicación de admisión ${selected.name}?`) && deleteLocationMutation.mutate(selected.uuid)} /><Button icon="pi pi-plus" label="Agregar sala" onClick={() => openLocation(undefined, selected.uuid)} /></div></div><div className="admin-location-cards">{children.map((item) => <button key={item.uuid} type="button" onClick={() => setSelectedUuid(item.uuid)}><i className="pi pi-th-large" /><strong>{item.name}</strong>{item.description && <span>{item.description}</span>}</button>)}</div></>}
              {selected && isWard && <><div className="admin-breadcrumb"><button onClick={() => setSelectedUuid(undefined)}>Ubicaciones de admisión</button><i className="pi pi-angle-right" /><button onClick={() => setSelectedUuid(selected.parentUuid)}>{locations.data?.find((item) => item.uuid === selected.parentUuid)?.name}</button><i className="pi pi-angle-right" /><span>{selected.name}</span></div><div className="admin-section-title"><div><h3>{selected.name}</h3><p>Distribución física de camas</p></div><div><Button outlined icon="pi pi-pencil" label="Editar sala" onClick={() => openLocation(selected)} /><Button outlined severity="danger" icon="pi pi-trash" label="Eliminar sala" onClick={() => window.confirm(`¿Está seguro de que desea eliminar la ubicación de admisión ${selected.name}?`) && deleteLocationMutation.mutate(selected.uuid)} />{Boolean(layout.data?.rows) && <Button outlined severity="danger" icon="pi pi-times" label="Eliminar distribución" onClick={() => window.confirm(`¿Está seguro de que desea eliminar la distribución de ${selected.name}?`) && layoutMutation.mutate({ uuid: selected.uuid, rows: 0, columns: 0 })} />}<Button icon="pi pi-th-large" label={layout.data?.rows ? "Editar distribución" : "Definir distribución"} onClick={() => { setFormErrors({}); setLayoutDraft({ rows: layout.data?.rows || 1, columns: layout.data?.columns || 1 }); }} /></div></div>
