...
import { getBahmniErrorTechnicalDetails } from "@/services/bahmni/http";
...
function ProgramEnrollmentPanel({ open, patientUuid, activeProgramUuids, onToggle }: { open: boolean; patientUuid: string; activeProgramUuids: Set<string>; onToggle(): void }) {
  ...
  return <section className="program-section program-enrollment-section"><header><h2>Enrolamiento en nuevo tratamiento</h2><Button text label={open ? "Cerrar" : "Abrir"} icon={open ? "pi pi-minus" : "pi pi-plus"} onClick={onToggle} /></header>
    ...
    {selectedAlreadyActive && <p className="error-banner">Paciente ya inscrito en el programa.</p>}{enroll.isError && <div className="error-banner"><p>No fue posible inscribir al paciente. Revise los datos e intente nuevamente.</p><details><summary>Detalles técnicos seguros</summary><pre>{JSON.stringify(getBahmniErrorTechnicalDetails(enroll.error), null, 2)}</pre></details></div>}
