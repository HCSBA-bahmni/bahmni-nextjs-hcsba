...
- El distintivo de cama en listados y perfiles no interpreta `hasBeenAdmitted`: ese campo sólo expresa admisión vigente. La lista IPD, la búsqueda clínica y la información del paciente muestran cama exclusivamente después de que `GET /beds?patientUuid=...` confirma la asignación actual. Un paciente admitido sin cama se identifica textualmente y las acciones ADT ofrecen Admitir/asignar cama; Transferir y Dar de alta sólo se habilitan con cama actual, como las guardas `bedDetails` legacy.
...
- Alta: exige visita y cama activas, envía `bahmnicore/discharge` con tipo `DISCHARGE`, invalida la sala de origen y el resumen de visita, y confirma dos efectos independientes: que el paciente ya no tenga cama y que `visit/summary` contenga `dischargeDetails.uuid`. Liberar la cama sin encuentro de alta es un resultado parcial y nunca se informa como alta exitosa.
