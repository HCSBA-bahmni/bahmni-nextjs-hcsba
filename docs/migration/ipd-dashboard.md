...
- `DC` cruza dos fuentes igual que legacy: las órdenes programadas de `/ipdVisit/{visitUuid}/medication` definen todas las filas y `/ipd/schedule/type/medication?view=drugChart` aporta los slots del turno. Por ello un medicamento sigue visible aunque no tenga dosis dentro del turno revisado. La línea temporal siempre representa el turno completo en intervalos de 30 minutos; no se reduce a las horas que contienen actividades. Las administraciones completadas se ubican por su hora efectiva y los demás estados por su hora programada.
