...
| Administración | Sí | Parcial: dashboard, Audit Log y Beds caracterizados; ver `migration/admin-audit-log.md` y `migration/admin-beds.md` | Parcial: dashboard, Audit Log y Beds nativos | Sí local, incluidos settings, jerarquía y contratos CRUD | Sí local Edge con mutaciones y accesibilidad | No: falta campaña HCSBA real y los demás estados | No; corte y enlaces en PR compañeros Docker/config pendientes de fusión, 6 estados AngularJS restantes |
